...
def Update(sModpath):
    ...
    # pprint.pprint(dInfo)
    #把旧对象替换回到原来的名称空间去
    for sName,oldObj in oldNameSpace.iteritems():
...
def DoAutoUpdate():
    ...
    try:
        for sPyFile in sLine.split(";"):#,一行内可以有多个文件,多个文件间用分号;分隔
            ...
            if glCurrentNeedUpdateFile.has_key(sPyFile):
...
if not globals().has_key("glDetailedUpdateFile"):
    glDetailedUpdateFile=[]

if not globals().has_key("glCurrentNeedUpdateFile"):
    glCurrentNeedUpdateFile={}

if not globals().has_key("gbUpdateScript"):
