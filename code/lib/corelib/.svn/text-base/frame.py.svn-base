...
class MainFrame(object):
    ...
    def finish_start(self, addr, app_name, names):
        self.application.pub(MSG_APP_FSTART, app_name, addr, names)
    ...
    def get_addr_names(self):
        return [(sub_proxy._app_name, addr, sub_proxy._mode, sub_proxy._app_names) for addr, sub_proxy in self.sub_proxys.iteritems()]
    ...
    def add_sub_proxy(self, addr, mode, app_name, names):
        ...
        for other_sub_proxy in self.sub_proxys.itervalues():
    ...
    def del_sub_proxy(self, sub_proxy):
        ...
        if not self.stoped:
            ...
            for sub_frame in self.sub_proxys.itervalues():
                spawn(sub_frame.del_other_proxy, app_name, addr, names)

    def stop(self):
        self.stoped = True
        for sub_frame in self.sub_proxys.values()[::-1]:
            try:
    ...
    def before_stop(self):
        """ 通知所有功能模块准备关闭 """
        self.cell_mgr.pause()
        for sub_proxy in self.sub_proxys.values()[::-1]:
            try:
...
class SubFrame(object):
    ...
    def start(self):
        ...
        self.main_proxy.finish_start(self.addr, self.name, names)

    def after_all_sub_start(self):
        for name, obj in self.names.items():
            if hasattr(obj, "after_all_start"):
                obj.after_all_start()
        pass

    def app_after_start(self):
        log.info("[subFrame] app_after_start:%s",self.addr)
        for name, obj in self.names.items():
            if hasattr(obj, "after_start"):
                obj.after_start()
...
class CellMgr(object):
    ...
    def _new_subgame(self, app_name, sub_cmd=None, cwd=None, kw=None):
        ...
        if self.config.debug_key == app_name:
            cmd += ' subgame_debug'
            times = 50
        elif app_name in self.config.ldebug_key:
            cmd += ' subgame_debug'
            times = 50
        if cwd is None:
