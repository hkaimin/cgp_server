...
def id2_id(obj):
    ...
    if isinstance(obj, dict):
        return dict([(id2_id(k), id2_id(v)) for k, v in obj.iteritems()])
    if obj == 'id':
        return '_id'
    return obj
...
class SaveProc(BaseProc):
    ...
    def simple(store, tname, values):
        ...
        if _id is None:
            ...
            if tname == "player":
                channel = data.pop("channel", 0)
                #补足2位
                if channel < 10:
                    channel = "0" + str(channel)
                else:
                    channel = str(channel)
                _id = int(str(_id) + channel + config.serverNo)
            else:
    ...
    def multi(store, tname, objs, insert):
        ...
        if insert:
            ...
            ids = xrange(max_id-l+1, max_id+1).__iter__()
...
def test_join():
    con = pymongo.MongoClient('mongodb://172.16.40.2/test')
