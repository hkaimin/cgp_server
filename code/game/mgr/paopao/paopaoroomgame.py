...
def room_capacity_check(cell):
    ...
    paopao_room_pool = config.paopao_room_pool
    if c < paopao_room_pool:
        return 1
    elif c == paopao_room_pool:
        return 0

    try:
        count = Game.rpc_paopao_room_mgr.get_player_count()
        c = Game.rpc_paopao_room_mgr.get_player_count()
    except:
        log.log_except()
        return 0

    total = c * config.paopao_room_players
    rate = float(total - count) / config.paopao_room_players
...
class PaopaoRoomGame(object):
    _rpc_name_ = 'rpc_paopao_room_game'
