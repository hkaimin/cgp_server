...
from corelib.data import json_loads, json_dumps
...
class Processer(object):
    ...
    def call(self, fname, data, noresult=False, timeout=None):
        ...
        data = json_dumps({FIELD_FUNC: fname,
                           FIELD_STATUS: 1,
                           FIELD_DATA: data,
                           FIELD_ERROR: ''},
                          ensure_ascii=True)
        mid = self._call_id.next()
        gw.send(self.pid, self.route_id, '%s%s' % (struct.pack(DATA_HEAD_STRUCT, TAG_REQ, mid), data))
        if noresult:
            return
        result = AsyncResult()
    ...
    def resp(self, mid, fname, status, msg, error_id):
        ...
        if fname in exclude:
            return 
        if status == RES_OK:
            data = json_dumps({FIELD_FUNC: fname,  # fname,
                               FIELD_STATUS: status,
                               FIELD_DATA: msg,
                               FIELD_ERROR: ''},
                              ensure_ascii=True)
            ...
            data = json_dumps({FIELD_FUNC: fname,  # fname,
                              ...
                              ensure_ascii=True)
        gw.send(self.pid, self.route_id, '%s%s' % (struct.pack(DATA_HEAD_STRUCT, TAG_RESP, mid), data))

    def router(self, addr_fnams):
        """ 添加gateway route (DFW的方法) """
        gw = self.gw_mgr.rpc_gws[self.gwid]
        gw.router(self.pid, addr_fnams)

    def add_router(self, routerID, addr):
        """ 添加gateway route (原始方法) """
        gw = self.gw_mgr.rpc_gws[self.gwid]
        gw.router(self.pid, routerID, addr)

    def remove_router(self, routerID):
        """ 移除gateway route """
        log.debug('[gateway]remove_router:%s', routerID)
        gw = self.gw_mgr.rpc_gws[self.gwid]
        print "------------->> remove_router addr"
        gw.RRouter(self.pid, routerID)

    def remove_router_old(self, addr):
        """ 移除gateway route """
    ...
    def _handle_request(self, tag, procId, params):
        ...
        #print "------------>>>fname:",tag,procId,fname,self.export,self
        ...
        try:
            ...
            if not ok:
                ...
                if data is None:
                    data = '[]'
                # print "========resp=====",procId, fname, RES_OK, data, 0
    ...
    def _handle(self):
        try:
            while 1:
                ...
                if msg is None:
                    break
                tag, procId = struct.unpack(DATA_HEAD_STRUCT, msg[:DATA_HEAD_LEN])
                params = json_loads(msg[DATA_HEAD_LEN:])
                if tag == TAG_RESP:
...
class GateWay(object):
    ...
    def on_close(self, rpc=None):
        """ 网关断开 """
        log.error(u'******网关断开连接,将通知所有Processer')
        self.stop()
...
class GateWayMgr(AbsExport):
    ...
    def gw_close(self, rpc_gw):
        ...
        log.warn(u'网关(%s - %s)断开', rpc_gw.get_addr(), close_gwid)
    ...
    def broadcast(self, fname, data, sendInfo):
        ...
        for gwid, group in sendInfo.iteritems():
