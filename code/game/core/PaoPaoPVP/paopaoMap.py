...
class PaopaoMap(object):
    def __init__(self, mapKey=0, bgconf=[], layerconf=[]):
        # print_stack()
        # import traceback
        # traceback.print_stack()
        print "------------------PaopaoMap", mapKey
    ...
    def baoItem(self, idx):
        ...
        if self.itemSpeed > 0: # 速度
            ...
            isSpeed = RandomKey({1:speedPer,0:(100-speedPer)}, 100)#random.randint(0, 1)
            if isSpeed:
                self.itemSpeed -= 1
        ...
        if self.itemNum > 0: # 泡泡数量道具
            ...
            isitemNum = RandomKey({1:itemNumPer,0:(100-itemNumPer)}, 100)#random.randint(0, 1)
            if isitemNum:
                self.itemNum -= 1
        ...
        if self.itemPower > 0: # 泡泡威力道具
            ...
            isitemPower = RandomKey({1:itemPowerPer,0:(100-itemPowerPer)}, 100)#random.randint(0, 1)
            if isitemPower:
                self.itemPower -= 1
