...
class APPConfig(object):
    ...
    def __init__(self):
        ...
        if sys.platform in ['win32', 'cygwin']:
            # self.debug_key = 'app_%s' % ('paopao_room1')
            self.ldebug_key = ["app_common", "app_logic1", "app_paopao_room1"]
            # self.ldebug_key = [""]
        #死循环检查功能
    ...
    def _get_config(self):
        ...
        if not exists(config_root_path):
            err = u'全局配置目录不存在:%s' % config_root_path
            print(err)
            os.makedirs(config_root_path)

        config_name = sys.argv.pop(2)
        sys.argv.pop(1)
    ...
    def load_config(self):
        ...
        cfg_dict = {}
        exec cfg_data in globals(), cfg_dict

        #加载app config
        app = cfg_dict.get('app')
        if app is None:
            raise ValueError(u'app在config文件中未配置')
...
class Launcher(object):
    ...
    def init_lib(self):
        """动态加载库"""
        # from package import install_pypy
        # install_pypy()
    ...
    def init_platform(self):
        ...
        if is_cygwin and self.sys_setting.sys_encoding not in ['UTF8']:
            log.locale_encode = 'UTF8'
    ...
    def execute(self, app):
        ...
        # print app_config
        # print "--------------2"
        # print self.sys_setting.root_path, 'app', app
        # print "--------------3"
        # print join(self.sys_setting.root_path, 'app', app)
        # print "--------------4"
