...
class Application(BaseApp):
    ...
    def _start_gw(self):
        import SysSetting
        port = config.port
        gw_path = join(dirname(SysSetting.root_path), 'gateway')
        print "gw_path:", gw_path
        if sys.platform == 'win32':
            gw_exe = join(gw_path, 'bin', config.win_exe)
        else:
            gw_exe = join(gw_path, 'bin', config.linux_exe)
    ...
    def go_get(self, *args):
        ...
        pkgs = [
            ...
            ('bitbucket.org/efun/gateway_ppt', ''),
        ]
...
def main():
    log.info('[gateway_ppt] start at:%s', config.port)
    app = Application()
    daemon(app)
